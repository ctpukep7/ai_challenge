...
class Day02ApiTest(unittest.TestCase):
    ...
    def test_base_request_disables_thinking_by_default(self):
    ...
    def test_multiple_stops_add_instruction_and_api_parameters(self):
        response, request_kwargs = self.post_with_client(
            {"stop_sequences": ["<<<END>>>", "<<<SECOND_END>>>"]}
        )

        self.assertEqual(response.status_code, 200)
        self.assertEqual(request_kwargs["stop"], ["<<<END>>>", "<<<SECOND_END>>>"])
    ...
    def test_settings_validation_and_missing_key_are_safe(self):
        ...
        too_many_stops = self.client.post(
            "/ask",
            json={
                "prompt": "Вопрос",
                "settings": {"stop_sequences": [f"stop-{index}" for index in range(17)]},
            },
        )
        self.assertEqual(too_many_stops.status_code, 400)
        self.assertIn("не более 16", too_many_stops.get_json()["error"])
    ...
    def test_page_preview_does_not_expose_key(self):
        ...
        self.assertIn("data-add-stop", page)
